...
let audio = new Audio(new URL('../audio/ww_get_rupee.mp3', document.currentScript?.src || window.location.href).href);

let redBoxRespawning = false; 

function spawnRedBoxRandom() {
    const box = GAME_SCREEN.redbox;
    const surface = GAME_SCREEN.surface;

    if (redBoxRespawning) {
        return
    }; 

    redBoxRespawning = true;      
    box.style.opacity = '0';      

    const x = Math.random() * (surface.clientWidth - box.clientWidth);
    const y = Math.random() * (surface.clientHeight - box.clientHeight);
    box.style.left = x + 'px';
    box.style.top = y + 'px';

    PLAYER.coinCount = (PLAYER.coinCount || 0) + 1;
    audio.play();

    setTimeout(() => {
        box.style.opacity = '1';
        redBoxRespawning = false; 

        GAME_SCREEN.debug_output.innerHTML =
            `<img src="./img/greenRupy.png" alt="Count"> ${PLAYER.coinCount}`; //Wichtig Für Später!!!!!!!
    }, 1000);
}
